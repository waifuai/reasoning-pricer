# GRAPE - Grape Network

## Overview

| Field | Value |
|-------|-------|
| **Symbol** | `GRAPE` |
| **Name** | Grape Network |
| **Token Type** | Protocol Utility Token |
| **Risk Class** | Class B (Systemic) |
| **Insider Score** | 45/100 |
| **Tariff Override** | 4% |
| **Calculated Tariff** | 10% |
| **Exchange Multiplier** | 1.1x |

## Risk Analysis

### Token Type
Protocol Utility Token

### Archetype
Class B (Systemic)

### Insider Score
**45/100** - Medium insider control

### Exchange Rate
**1.1x** (Tariff: 10%)

## Tags

- infrastructure
- social-gating
- dao-tools
- community

## Analyst Notes

The standard for token-gated communities on Discord. Essential infrastructure for DAOs, though its token economics have struggled to capture value commensurate with its utility.

---

## Reasoning Pricer Analysis

### Post-Fiat Collapse Valuation

| Metric | Value |
|--------|-------|
| **Token Type** | ProtocolUtility |
| **Base Multiplier Range** | 10x - 25x |
| **Type Adjustment Factor** | 18x |
| **Risk Class Adjustment** | 1.0x |
| **Insider Risk Factor** | 0.78x |
| **AI Timeline Factor** | 1.80x |
| **AI Category** | Passive Utility |
| **Current AI Phase** | Creative Renaissance |
| **Real Valuation Multiplier** | 5x |
| **Uncertainty Range** | 3x - 6x |
| **Current vs Real Price** | $1.00 → $4.90 |
| **Trading Signal** | HOLD |

### Analysis
[AI Timeline: Creative Renaissance] Asset classified as Passive Utility (Standard utility tokens with limited AI integration potential. Moderate performance through AI timeline.). AI Timeline Factor: 1.80x (AI acceleration boosts this asset significantly). Protocol Utility Token: benefits from network effects and protocol utility (base range: 10x - 25x). Insider control (45/100) reduces multiplier by 0.78x due to centralization risks. Rank #450 imposes 0.2x penalty (illiquid, capital flight crushes value). Final real valuation multiplier: 4.9x. Token trading at $1.00 today would be worth $4.88 post-fiat collapse. AI acceleration provides strong tailwinds. Trading signal (HOLD): Moderate value post-collapse. Maintain current position.

---

## Navigation

- [📊 Risk Class Summary](../summaries/class-b-systemic.md)
- [🏠 Main Index](../index.md)
- [📈 All Tokens](../tokens/index.md)
