# Class D (Speculative) - Summary

## Overview

This report summarizes all tokens classified as **Class D (Speculative)**.

### Statistics

| Metric | Value |
|--------|-------|
| **Total Tokens** | 1 |
| **Average Insider Score** | 5.0/100 |
| **Average Tariff** | 200.0% |
| **Average Exchange Multiplier** | 3.00x |

## Token Details

| Symbol | Name | Insider Score | Tariff | Exchange Rate |
|--------|------|---------------|--------|---------------|
| [LFG](../tokens/lfg.md) | Less Fn Gas | 5 | 200.0% | 3.00x |

## Risk Characteristics

**Characteristics:**
- Very high insider control (90+ score)
- Experimental or highly speculative
- Maximum regulatory and execution risks
- High volatility and uncertainty

**Investment Thesis:** Maximum risk/reward for sophisticated investors only.

## Tag Analysis

| Tag | Frequency |
|------|-----------|
| onboarding | 1 |
| community | 1 |
| fee-reduction | 1 |

---

## Navigation

- [🏠 Main Index](../index.md)
- [📊 All Risk Classes](./)
- [🔗 Individual Token Reports](../tokens/)
